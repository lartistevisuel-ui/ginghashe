...
export async function getCategoriesFromDB() {
  ...
  try {
    const res = await fetch(
      `${URL}/rest/v1/categories?select=*&order=position.asc`,
      { headers: baseHeaders(), cache: "no-store" }
    );
    if (!res.ok) return null;
    return await res.json();
...
export async function addCategoryToDB(cat) {
  if (!isDbConfigured()) throw new Error("Base non configurée.");
...
export async function updateCategoryInDB(id, fields) {
  if (!isDbConfigured()) throw new Error("Base non configurée.");
  const res = await fetch(
    `${URL}/rest/v1/categories?id=eq.${encodeURIComponent(id)}`,
    {
      method: "PATCH",
      headers: { ...baseHeaders(), Prefer: "return=representation" },
...
export async function getVerification(chatId) {
  if (!isDbConfigured()) return null;
  try {
    const res = await fetch(
      `${URL}/rest/v1/verifications?chat_id=eq.${encodeURIComponent(chatId)}&select=*`,
      { headers: baseHeaders(), cache: "no-store" }
    );
    if (!res.ok) return null;
    const rows = await res.json();
...
export async function upsertVerification(chatId, code) {
  if (!isDbConfigured()) return;
  await fetch(`${URL}/rest/v1/verifications`, {
    method: "POST",
    headers: { ...baseHeaders(), Prefer: "resolution=merge-duplicates" },
    body: JSON.stringify({
...
export async function setVerified(chatId) {
  if (!isDbConfigured()) return;
  await fetch(
    `${URL}/rest/v1/verifications?chat_id=eq.${encodeURIComponent(chatId)}`,
    {
      method: "PATCH",
      headers: baseHeaders(),
